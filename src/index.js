...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // HUB API - SOP LINKS
      // ============================================
      // List all SOPs (with optional category filter)
...
async function handleHubStats(request, env, corsHeaders) {
  try {
    ...
    // Get counts by type (all statuses, not just non-completed)
    ...
    // Also get total count (all cases)
...
async function handleHubCases(request, env, corsHeaders) {
  try {
    ...
    const assigneesResult = await env.ANALYTICS_DB.prepare(`
      SELECT DISTINCT assigned_to FROM cases WHERE assigned_to IS NOT NULL AND assigned_to != '' AND assigned_to != 'Unassigned'
    `).all();
    const assignees = (assigneesResult.results || []).map(r => r.assigned_to).filter(Boolean);

    // Get counts by type for navigation badges
...
async function verifyHubToken(token, env) {
  ...
  try {
    ...
    // Get user details from database
    const user = await env.ANALYTICS_DB.prepare(`
      SELECT id, username, name, email, role, is_active FROM admin_users WHERE username = ?
    `).bind(payload.username).first();
...
async function handleHubUnifiedSearch(request, env, corsHeaders) {
  try {
    ...
    const casesResult = await env.ANALYTICS_DB.prepare(`
      ...
      FROM cases
      WHERE customer_email LIKE ? OR customer_name LIKE ? OR order_number LIKE ? OR case_id LIKE ?
      ORDER BY created_at DESC
      LIMIT ?
    ...
    const sessionsResult = await env.ANALYTICS_DB.prepare(`
      ...
      FROM sessions
      WHERE customer_email LIKE ? OR customer_name LIKE ? OR order_number LIKE ?
      ORDER BY started_at DESC
      LIMIT ?
...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Animated Gradient Orbs for Page Content */
    .page-content::before {
      content: '';
      position: fixed;
      top: 0;
    ...
    .stat-card.highlight {
      ...
      color: white;
      border: none;
      box-shadow: 0 12px 40px rgba(26, 54, 93, 0.25), 0 4px 12px rgba(26, 54, 93, 0.15);
    }

    .stat-card.highlight:hover {
      box-shadow: 0 16px 48px rgba(26, 54, 93, 0.3), 0 6px 16px rgba(26, 54, 93, 0.2);
      transform: translateY(-4px);
    }
      ...
      <div class="page-content">
        ...
        <div id="sopView" style="display: none;">
          <div class="cases-card">
            ...
            <!-- Category Tabs -->
            <div style="padding: 16px 24px; border-bottom: 1px solid var(--gray-200); display: flex; gap: 8px; flex-wrap: wrap;">
              <button class="sop-tab active" data-category="" onclick="HubSOP.filterByCategory('')">All</button>
              <button class="sop-tab" data-category="refund" onclick="HubSOP.filterByCategory('refund')">Refunds</button>
              <button class="sop-tab" data-category="return" onclick="HubSOP.filterByCategory('return')">Returns</button>
              <button class="sop-tab" data-category="shipping" onclick="HubSOP.filterByCategory('shipping')">Shipping</button>
              <button class="sop-tab" data-category="subscription" onclick="HubSOP.filterByCategory('subscription')">Subscriptions</button>
              <button class="sop-tab" data-category="manual" onclick="HubSOP.filterByCategory('manual')">Manual</button>
            </div>
        ...
        <div id="emailTemplatesView" style="display: none;">
          <div class="cases-card">
            ...
            <!-- Category Tabs -->
            <div style="padding: 16px 24px; border-bottom: 1px solid var(--gray-200); display: flex; gap: 8px; flex-wrap: wrap;">
              <button class="template-tab active" data-category="" onclick="HubEmailTemplates.filterByCategory('')">All</button>
              <button class="template-tab" data-category="refund" onclick="HubEmailTemplates.filterByCategory('refund')">Refunds</button>
              <button class="template-tab" data-category="return" onclick="HubEmailTemplates.filterByCategory('return')">Returns</button>
              <button class="template-tab" data-category="shipping" onclick="HubEmailTemplates.filterByCategory('shipping')">Shipping</button>
              <button class="template-tab" data-category="subscription" onclick="HubEmailTemplates.filterByCategory('subscription')">Subscriptions</button>
              <button class="template-tab" data-category="manual" onclick="HubEmailTemplates.filterByCategory('manual')">Manual</button>
            </div>
