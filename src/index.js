...
async function handleHubIssues(request, env, corsHeaders) {
  try {
    ...
    // Try to get issues from issue_reports table (renamed from trouble_reports)
    let issues = [];
    try {
      const result = await env.ANALYTICS_DB.prepare(`
      ...
      // Try old table name for backwards compatibility
      try {
        const result = await env.ANALYTICS_DB.prepare(`
          SELECT * FROM trouble_reports ORDER BY created_at DESC LIMIT ? OFFSET ?
        `).bind(limit, offset).all();
        issues = result.results || [];
      } catch (e2) {
        console.log('Issue reports table may not exist:', e2.message);
      }
    }
    ...
    const countResult = await env.ANALYTICS_DB.prepare(
      ...
      return env.ANALYTICS_DB.prepare(
        `SELECT COUNT(*) as count FROM trouble_reports`
      ).first();
    });
          ...
          <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
            ...
            <div style="position: relative; flex: 1; min-width: 300px; max-width: 500px;">
              <div class="search-box" id="searchBoxContainer">
                ...
                <input type="text" placeholder="Search anything: name, email, case ID, order, resolution..." id="searchInput" autocomplete="off">
